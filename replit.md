...
- **Apple Wallet Pass Delivery (iOS)**: Server generates `.pkpass`, stores binary buffer with UUID, returns JSON with `downloadPath`. On iOS Xcode builds, client uses native `BladeWalletPass` Expo module (`modules/blade-wallet-pass/`) which wraps `PKAddPassesViewController` — downloads the `.pkpass` from the server URL, creates a `PKPass` object, and presents Apple's native "Add to Wallet" sheet directly inside the app (Apple's method #1: in-app). Falls back to local download + share sheet if native module unavailable. Download entries kept for 30s after first access to allow retries. On Android, downloads locally and uses share sheet.
