...
- **Apple Wallet Pass Delivery (iOS)**: Server generates `.pkpass`, stores binary buffer with UUID, returns JSON with `downloadPath`. On iOS Xcode builds, client uses native `BladeWalletPass` Expo module (`modules/blade-wallet-pass/`) which wraps `PKAddPassesViewController` — the Swift module downloads the `.pkpass` binary via `URLSession`, creates a `PKPass` object, checks `PKPassLibrary.containsPass()` for duplicates, then presents `PKAddPassesViewController` with a `PKAddPassesViewControllerDelegate` that resolves the JS promise with `{presented, added, alreadyInWallet}` when the user finishes. The delegate uses `objc_setAssociatedObject` to prevent deallocation. Falls back to local download + share sheet if native module unavailable. Download entries kept for 30s after first access to allow retries. On Android, downloads locally and uses share sheet.
- **Native File Download Pattern**: For PDFs and non-wallet files on native platforms (iOS/Android Xcode builds), server buffers the file and returns JSON with `{ downloadPath: "/api/passport/download/{uuid}" }` → client uses `FileSystem.downloadAsync(url, localPath)` to save the file natively → share via `expo-sharing`. Temporary downloads expire after 5 minutes. This pattern is used for trip report PDFs (`/api/trip/report`) and passport PDFs (`/api/passport/pdf`). Web platform continues using base64 data transfer.
