...
- **Apple Wallet Pass Delivery (iOS)**: Server generates `.pkpass`, stores binary buffer with UUID, returns JSON with `downloadPath`. On iOS, client uses `Linking.openURL(downloadUrl)` — the HTTPS URL serves `Content-Type: application/vnd.apple.pkpass` with `Content-Disposition: inline`, which triggers iOS Safari's native "Add to Wallet" sheet (Apple's method #2: web download). Falls back to local download + share sheet if Linking fails. Download entries kept for 30s after first access to allow retries. On Android, downloads locally and uses share sheet.
