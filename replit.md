...
- **Firmware OTA (FOTA v2.0)**: Uses HALO FOTA v2.0 protocol. Enter bootloader via `$APP_CONFIG,UPDATE_FW` software command. Flow: HELLO (0x7F) → FW_INFO (0x10, start=0x08004000) → ERASE (0x43) → WRITE (0x31, 256-byte blocks) → GOTOAPP (0x21). Supports .bin and .hex files. ACK=0x79, NACK=0x1F. FW_INFO sends start address and firmware size in LSB (Little-Endian) byte order. ACK timeout: 1000ms general, 1500ms per write block, 15000ms for erase. Bootloader entry includes RX buffer drain, 2500ms reset delay, 1500ms init delay, and auto-retry of UPDATE_FW on attempt 3 of 5.
- **FOTA Speed Optimization (Feb 2026)**: Removed all redundant TX delays — no per-chunk delays, no post-block delays, no sendBinaryData TX delay. Blocks sent as single 256-byte writes via BT stack (was 4×64-byte chunks with 22ms delays each). ACK-based flow control is the sole pacing mechanism. `waitForAck` uses polling loop to handle multi-byte responses (scans for ACK/NACK across all received bytes). OTA RX buffer in all BT services (Classic + BLE, both apps) fixed to clear buffer atomically when resolving to prevent stale data accumulation. INTER_STEP_DELAY_MS reduced from 300ms to 50ms. Expected per-block time: ~100-110ms (vs ~1.9s before).
