 ...
 * Parses .hex files into memory blocks for firmware flashing
 */
...
export function splitIntoBlocks(data: Uint8Array, blockSize: number = 256): Uint8Array[] {
  const blocks: Uint8Array[] = [];
  
  for (let i = 0; i < data.length; i += blockSize) {
    const end = Math.min(i + blockSize, data.length);
    const block = new Uint8Array(end - i);
