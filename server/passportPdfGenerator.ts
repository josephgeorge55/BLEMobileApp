...
export async function generatePassportPDFBuffer(passportData: PassportData): Promise<Buffer> {
  const doc = new PDFDocument({
    size: 'A4',
    layout: 'portrait',
    margins: { top: 30, bottom: 30, left: MARGIN, right: MARGIN },
    autoFirstPage: false,
    bufferPages: true,
    info: {
  ...
  const qrTextX = MARGIN + qrSize + 12;
  const qrTextWidth = CONTENT_WIDTH - qrSize - 12;
  ...
  doc.text(
    ...
    qrTextX, y + 14, { width: qrTextWidth, lineGap: 1 }
  );
  y += qrSize + 10;
  ...
  doc.text('Blade Marine Technologies Ltd', MARGIN, footerY + 5, { width: CONTENT_WIDTH, align: 'center' });
  ...
  doc.text(
    ...
    MARGIN, footerY + 14, { width: CONTENT_WIDTH, align: 'center' }
  );

  const now = new Date();
  const utcStamp = now.toISOString().replace('T', ' ').replace(/\.\d+Z$/, ' UTC');
  doc.font('Helvetica').fontSize(5.5).fillColor(MID_GRAY);
  doc.text(`Generated: ${utcStamp}  |  bladeoutboards.com`, MARGIN, footerY + 22, { width: CONTENT_WIDTH, align: 'center' });
